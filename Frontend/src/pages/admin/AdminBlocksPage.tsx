...
const statusStyles: Record<string, { label: string; className: string; icon: any; dotColor: string }> = {
  PENDING: {
    label: 'Pending',
...
export default function AdminBlocksPage() {
  ...
  const stats = {
    ...
    pending: blocks.filter(b => b.status === 'PENDING' || b.status === 'IN_DEVELOPMENT').length
  };
        ...
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-lg font-bold text-slate-900 tracking-tight block leading-none">Admin</span>
      ...
      <main className="container mx-auto px-4 py-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-lg">Manage and organize your content blocks.</p>
          </div>
          ...
          <div className="p-0">
            {loading ? (
              ...
              <div className="text-center py-32 px-4">
                ...
                <h3 className="text-xl font-bold text-slate-900">No blocks found</h3>
                ...
                <div className="hidden md:block overflow-x-auto">
                  <table className="w-full text-sm text-left">
                    <thead className="bg-slate-50/50 border-b border-slate-100 text-xs uppercase text-muted-foreground font-semibold tracking-wider">
                      <tr>
                        <th className="px-8 py-5 w-[45%]">Block Details</th>
                          ...
                          >
                            <td className="px-8 py-5">
                              <div className="flex items-center gap-5">
                                ...
                                <div>
                                  <div className="font-semibold text-slate-900 text-base group-hover:text-primary transition-colors">{block.title}</div>
                                  <div className="text-xs text-muted-foreground font-mono mt-1 bg-slate-100 px-2 py-0.5 rounded-md inline-block">
                                    /{block.slug}
                                  </div>
                                </div>
                              </div>
                            ...
                            <td className="px-6 py-5">
                              <Badge variant="outline" className={`gap-2 pl-2 pr-3 py-1.5 rounded-full border-0 shadow-sm ring-1 ring-inset ${statusStyles[block.status]?.className}`}>
                                <span className={`w-1.5 h-1.5 rounded-full ${statusStyles[block.status]?.dotColor} animate-pulse`} />
                                {statusStyles[block.status]?.label}
                              </Badge>
                            </td>
                              ...
                              <div className="flex items-center justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                                ...
                                <DropdownMenu>
                                  ...
                                  <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-xl border-slate-100">
                                    ...
                                    <DropdownMenuItem onClick={() => handleArchive(block.id)} className="cursor-pointer">
                                      <Archive className="w-4 h-4 mr-2 text-slate-500" />
                                      Archive Block
                                    </DropdownMenuItem>
                                    <DropdownMenuSeparator />
                                    <DropdownMenuItem className="text-red-600 focus:text-red-600 focus:bg-red-50 cursor-pointer" onClick={() => handleDelete(block.id)}>
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <div className="font-semibold text-slate-900 text-lg">{block.title}</div>
                              <div className="text-xs text-muted-foreground font-mono bg-slate-100 px-2 py-0.5 rounded-md inline-block mt-1">/{block.slug}</div>
                            </div>
                          </div>
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                            ...
                            <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-xl">
                              <DropdownMenuItem asChild>
                                <Link to={`/admin/blocks/${block.id}/edit`} className="cursor-pointer">
                        ...
                        <div className="flex items-center justify-between text-sm pt-2">
                          <Badge variant="outline" className={`gap-1.5 pl-1.5 pr-2.5 py-1 rounded-full border-0 shadow-sm ring-1 ring-inset ${statusStyles[block.status]?.className}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${statusStyles[block.status]?.dotColor}`} />
                            {statusStyles[block.status]?.label}
                          </Badge>
